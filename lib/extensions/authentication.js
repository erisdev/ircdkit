...
module.exports = function (server) {
	...
	server.on('connection', function (connection) {
		...
		connection.pauseAuthentication = reason => {
			debug('pausing authentication:', reason);
		...
		function queueable(callback) {
			return function (...args) {
				if (paused) {
					...
					debug('calling callback immediately');
					callback.apply(this, args);
				}
			}
		}

		connection.on('USER', queueable((username, hostname, servername, realname) => {
		...
		connection.on('PASS', queueable(password => {
